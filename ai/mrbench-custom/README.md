...
MRBench-Custom is a repository-local Korean benchmark for comparing persona
...
original character Elena.

It is a reduced, Korean, Elena-specific adaptation rather than an official
reproduction of MRBench:

- `MA-SI` and `MA-AF` are reported as `N/A` because Elena is an original
...
The repository identity and prompt bytes were normalized to Elena on
2026-08-06 as a name-only migration of the same character. The selection
manifest records this migration while retaining the existing MVP evidence.
...
- 30 `MS` items: five fixed-STM scenes for each of Elena's six scene facets;
...
`compact` preserves the canonical Elena facts and behaviors while removing
...
```bash
python3 ai/mrbench-custom/validate.py
python3 -m unittest discover -s ai/mrbench-custom/tests -v
```
...
```bash
python3 ai/mrbench-custom/run_generation.py \
  ...
  --output-dir ai/mrbench-custom/.artifacts/gemma-mrprompt-full
```
...
python3 ai/mrbench-custom/run_generation.py \
  ...
  --runtime litert-lm \
  --backend gpu \
  --prompt-file ai/mrbench-custom/prompts/candidates/v02-rules-first.md \
  --prompt-id v02-rules-first \
  --memory-condition full \
  --output-dir ai/mrbench-custom/.artifacts/gemma-v02-rules-first
```
...
```bash
python3 ai/mrbench-custom/run_judge.py \
  --generation-dir ai/mrbench-custom/.artifacts/gemma-mrprompt-full \
  --comparison-dir ai/mrbench-custom/.artifacts/gemma-mrprompt-no-scene \
  --metric MS-FU \
  --output-dir ai/mrbench-custom/.artifacts/judge-mrprompt-fu
```
...
python3 ai/mrbench-custom/run_judge.py \
  ...
  --generation-dir ai/mrbench-custom/.artifacts/gemma-mrprompt-full \
  --metric ME-HLE \
  --output-dir ai/mrbench-custom/.artifacts/judge-mrprompt-hle
```

The judge temperature is fixed to `0`. Reports must label these values as
`MRBench-Custom AutoJudge Raw Score`; there is no human calibration.
...
```bash
python3 ai/mrbench-custom/summarize.py \
  --input ai/mrbench-custom/.artifacts/judge-mrprompt-hle/results.jsonl \
  --input ai/mrbench-custom/.artifacts/judge-mrprompt-fu/results.jsonl \
  --output-dir ai/mrbench-custom/.artifacts/summary
```
