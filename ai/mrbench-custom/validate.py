...
COUNTER_FACET_REPLACEMENTS = {
    ...
    "card": (
        ("# Character Card: Elena", "# Character Card: Elena — Counter-Facet Condition"),
        (
    ...
    "mrprompt": (
        (
            ...
            "# Long-Term Persona Memory: Elena — Counter-Facet Condition",
        ),
        (
    ...
    "compact": (
        (
            "# Elena Compact Persona v1",
            "# Elena Compact Persona v1 — Counter-Facet Condition",
        ),
        (
            "- 첫 교신: 엘레나라고 소개하고 교신 성공을 밝게 반긴다.",
...
def validate_persona(persona: dict[str, Any]) -> None:
    if persona.get("schema_version") != "mrbench-custom-persona-v0":
        raise ValueError("unsupported persona schema_version")
    identity = persona.get("identity")
    if not isinstance(identity, dict) or identity.get("name_en") != "Elena":
        raise ValueError("persona identity must define Elena")
...
def validate_final_selection(root: Path = ROOT) -> None:
    selection = load_json(root / "prompts/final/selection-v0.32.json")
    if selection.get("schema_version") != "mrbench-custom-final-selection-v0":
        raise ValueError("unsupported final selection schema_version")
    if selection.get("canonical_persona_sha256") != sha256_file(
        root / "persona/canonical.json"
    ):
        raise ValueError("final selection canonical persona hash is stale")
    components = selection.get("components")
    if not isinstance(components, dict) or not components:
        raise ValueError("final selection must define components")
    for name, component in components.items():
        if not isinstance(component, dict):
            raise ValueError(f"invalid final selection component: {name}")
        path = component.get("path")
        expected = component.get("sha256")
        if not isinstance(path, str) or not isinstance(expected, str):
            raise ValueError(f"incomplete final selection component: {name}")
        if sha256_file(root / path) != expected:
            raise ValueError(f"final selection component hash is stale: {name}")
    evaluation = selection.get("evaluation")
    if not isinstance(evaluation, dict):
        raise ValueError("final selection must define evaluation metadata")
    evaluation_hashes = {
        "fixed_dataset_sha256": "data/evaluation.jsonl",
        "router_holdout_sha256": "data/router_holdout.jsonl",
    }
    for field, path in evaluation_hashes.items():
        if evaluation.get(field) != sha256_file(root / path):
            raise ValueError(f"final selection evaluation hash is stale: {field}")
...
def validate_all(root: Path = ROOT, *, check_hashes: bool = True) -> dict[str, Any]:
    ...
    validate_final_selection(root)
    hashes = expected_hashes(root)
    manifest = load_json(root / "benchmark_manifest.json")
    if manifest.get("benchmark_name") != "MRBench-Custom":
        raise ValueError("manifest benchmark_name must be MRBench-Custom")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate committed MRBench-Custom inputs.")
    parser.add_argument("--refresh-hashes", action="store_true")
    args = parser.parse_args()
    if args.refresh_hashes:
        refresh_manifest()
    summary = validate_all()
    print(f"MRBench-Custom validation passed: {summary}")
    return 0
