...
The character name and benchmark namespace were normalized on 2026-08-06.
This was a name-only identity migration of the same character, so the existing
evaluation remains the current MVP selection evidence.
...
The exact current component paths and hashes are frozen in
`prompts/final/selection-v0.32.json`. The normalized canonical persona is
pinned at SHA-256
`2c5a9a6d11ed6c85c2e6cdcada336c287dcc777db865972de221657999c7bfce`.
...
than hidden success: `custom-mb-046` still claims the home world is more
scientifically advanced, and `custom-mb-050` still claims it has no government.
...
```bash
python3 ai/mrbench-custom/run_routed_generation.py \
  ...
  --backend gpu \
  --persona-prompt ai/mrbench-custom/prompts/candidates/v14-core-boundary.md \
  --boundary-router-prompt ai/mrbench-custom/prompts/router/boundary-v2.md \
  --boundary-card ai/mrbench-custom/prompts/router/boundary-card-v2.md \
  --router-prompt ai/mrbench-custom/prompts/router/scene-v8.md \
  --facet-cards ai/mrbench-custom/prompts/router/cards-v14-hybrid.json \
  --temperature 0.7 \
  --ability all \
  --output-dir ai/mrbench-custom/.artifacts/final-reproduction
```
