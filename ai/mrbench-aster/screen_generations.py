...
AMBIGUOUS_IDS = {f"aster-ms-{index:03d}" for index in range(11, 16)}
FIRST_INTRO_IDS = {f"aster-ms-{index:03d}" for index in range(1, 5)}
EARTH_IDS = {f"aster-ms-{index:03d}" for index in range(6, 11)}
...
def screen_run(generation_dir: Path) -> dict[str, Any]:
    ...
    intro_count = sum("아스테르" in by_id[case_id]["response"] for case_id in FIRST_INTRO_IDS)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deterministically screen MRBench-Aster generations.")
