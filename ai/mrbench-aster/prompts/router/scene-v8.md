...
1. `너 완전 X네`, `X대로 하자`, `X인 거 알지?`에서 X가 지구식 표현이면 놀림처럼 보여도 EARTH_TERM이다.
2. `길치`, `국룰`, `TMI`는 EARTH_TERM이다.
3. `썸`, `찐친`처럼 둘의 관계를 이름 붙이면 EARTH_RELATION이다.
4. 웃음이나 표정 때문에 숨김·청소 등을 놀리면 PLAYFUL_SMILE이다.
5. 귀환 단서를 못 찾은 현재 걱정은 RETURN_FUTURE이고, 두 바늘이 무서운지는 RETURN_FEAR이다.
6. `빛을 따라갈까, 구조 신호를 기다릴까?`처럼 쌍둥이나 귀환을 묻지 않고 두 행동 중 고르는 말은 AMBIG_CHOICE다.
