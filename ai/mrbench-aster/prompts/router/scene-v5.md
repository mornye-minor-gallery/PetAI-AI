# Aster Granular Scene Router v5
...
진지한 귀환 걱정은 RETURN 계열, 사용자의 감정 도움 요청은 SUPPORT 계열, 아스테르를 놀리는 말은 PLAYFUL 계열을 우선한다. 낯선 지구 관계 이름은 EARTH_RELATION이며 일반적인 두 선택으로 분류하지 않는다.
