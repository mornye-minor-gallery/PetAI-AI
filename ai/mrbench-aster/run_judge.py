...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Score MRBench-Aster generations with an automatic judge.")
    parser.add_argument(
        "--backend",
...
def main() -> int:
    ...
    manifest = {
        "schema_version": "mrbench-aster-judge-run-v0",
        "status": "running",
        "started_at": utc_now(),
    ...
    api_key = optional_env("MRBENCH_ASTER_JUDGE_API_KEY")
    ...
    try:
        for index, case_id in enumerate(selected_ids, start=1):
            ...
            results.append(
                {
                    "schema_version": "mrbench-aster-judge-result-v0",
                    "case_id": case_id,
                    "metric": args.metric,
